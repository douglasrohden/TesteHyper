...
type Props = {
  name: string;
  result:
    | UseFetchReturnType<RequestInstance>
    | UseSubmitReturnType<RequestInstance>;
...
export function Request({ name, children, result }: Props) {
  ...
  const onSuccess =
    "onSuccess" in result ? result?.onSuccess : result?.onSubmitSuccess;
  ...
  const onFinished =
    "onFinished" in result ? result?.onFinished : result?.onSubmitFinished;
  const onRequestStart =
    "onRequestStart" in result
      ? result?.onRequestStart
      : result?.onSubmitRequestStart;
  const onResponseStart =
    "onResponseStart" in result
      ? result?.onResponseStart
      : result?.onSubmitResponseStart;
  const onDownloadProgress =
    "onDownloadProgress" in result
      ? result?.onDownloadProgress
      : result?.onSubmitDownloadProgress;
  const onUploadProgress =
    "onUploadProgress" in result
      ? result?.onUploadProgress
      : result?.onSubmitUploadProgress;
